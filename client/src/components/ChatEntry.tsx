...
export const ChatEntry = ({ entry, index }: { entry: HistoryMsg, index: number }) => {
    ...
    const entryFormatted = entry.content.replace("\n", "\n");
    ...
    if(entry.role === "user") {
       return (
        <div key={index} className={`relative chat-entry ${entry.role} ${expanded ? "expanded" : ""}`} data-role={entry.role} data-state={entry.state}>
            ...
            <CopyToClipboard text={entryFormatted} onCopy={(_text, result: boolean) => {
                if(timeoutRef.current) clearTimeout(timeoutRef.current);
                setCopied(result ? "Copied" : "Failed to Copy")
                timeoutRef.current = setTimeout(() => setCopied("Copy"), 3000);
            }}>
                <button
            ...
            <MarkdownWithCopyButton entry={entryFormatted} />
        </div>
        )
    }
    return (
